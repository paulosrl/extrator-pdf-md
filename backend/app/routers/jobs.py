...
@router.get("/{job_id}/download/rawtext")
async def download_rawtext(
    job_id: uuid.UUID,
    db: AsyncSession = Depends(get_async_db),
    current_user: User = Depends(get_current_user),
):
    ...
    if not job:
        raise HTTPException(status_code=404, detail="Job não encontrado")
    if job.status != JobStatus.done or not job.rawtext_path:
        raise HTTPException(status_code=404, detail="Texto bruto não disponível")

    filename = job.original_filename.rsplit(".", 1)[0] + "_rawtext.txt"
    return FileResponse(
        path=job.rawtext_path,
